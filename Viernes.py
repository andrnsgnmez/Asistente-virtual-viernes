...
comandos = """ 
    ...
    - Termina
"""
# Label del título y foto de viernes
viernes_photo = ImageTk.PhotoImage(Image.open("p22.png"))
back_photo = Label(main_window, image=viernes_photo).pack(pady=0)
# Canvas en dónde se ubicarán los comandos
canvas = Canvas(bg="#FFFFFF", height=450, width=200)
canvas.place(x=0, y=0)
canvas.create_text(90, 80, text=comandos, fill='black', font='Arial 11')
# Texto en dónde se mostrará información de Wikipedia
text_info = Text(main_window,  bg="#FFFFFF", fg="black")
text_info.place(x=0, y=175, height=350, width=204)
# Función de hablar
def talk(text):
    engine.say(text)
    engine.runAndWait()
...
# Función en donde viernes saluda al usuario
def say_hello():
    try:
...
def run_viernes():    
    while True:
        ...
        if 'reproduce' in rec:
            ...
            talk("Hasta pronto!")
            sub.call('taskkill /IM viernes.exe /F', shell=True)
        elif 'termina' in rec:
            talk('Estare ejecutandome en segundo plano, por si me necesita!')
            break
...
def open_file_window():
    ...
    button_add = Button(file_win, text="Agregar", bg='#FF0000', fg="white", width=8, height=1, command=add_files).pack(pady=5)
def open_page_window():
    ...
    button_add = Button(file_win, text="Agregar", bg='#FF0000', fg="white", width=8, height=1, command=add_sites).pack(pady=5)
def open_app_window():
    ...
    button_add = Button(file_win, text="Agregar", bg='#FF0000', fg="white", width=8, height=1, command=add_apps).pack(pady=5)
...
# Función para que viernes diga que cosas ha guardado el usuario
def talk_files():
    if bool(files) == True:
...
# Botones main_window
button_voice1 = Button(main_window, text="Voz México", bg='#FFFF00', fg="black", font=('Arial', 10, 'bold'), command=mexican_voice) \
    .place(x=645, y=90, height=30, width=100)
button_voice2 = Button(main_window, text="Voz España", bg='#0000CD', fg="white", font=('Arial', 10, 'bold'), command=spanish_voice) \
    .place(x=645, y=125, height=30, width=100)
button_voice3 = Button(main_window, text="Voz USA", bg='#FF0000', fg="white", font=('Arial', 10, 'bold'), command=english_voice) \
...
button_apps = Button(main_window, text="Agregar apps", bg='#FF0000', fg="white", font=('Arial', 10, 'bold'), command=open_app_window) \
...
button_add_p = Button(main_window, text="Páginas agregadas", bg='#FF0000', fg="white", font=('Arial', 7, 'bold'), command=talk_sites) \
    .place(x=355, y=405, height=30, width=100)
button_add_a = Button(main_window, text="Apps agregadas", bg='#FF0000', fg="white", font=('Arial', 7, 'bold'), command=talk_apps) \
    .place(x=475, y=405, height=30, width=100)
